<flux:main>
    <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
        ...
        {{-- Table Section --}}
        <div
            class="flex-1 overflow-hidden rounded-xl border border-neutral-200 bg-white dark:border-neutral-700 dark:bg-neutral-800">
                        ...
                        @forelse($agents as $agent)
                            <tr wire:key="agent-{{ $agent->id }}"
                                ...
                                <td class="px-4 py-3 font-mono text-indigo-600 dark:text-indigo-400">
                                    {{ $agent->agent_code }}</td>
                                <td class="px-4 py-3 font-medium text-zinc-900 dark:text-zinc-100">{{ $agent->agent_name }}
                                </td>
                                <td class="px-4 py-3 text-zinc-600 dark:text-zinc-400">{{ $agent->usertelegram ?? '-' }}
                                </td>
                                <td class="px-4 py-3 text-zinc-500 dark:text-zinc-400">
                                    {{ $agent->created_at->format('d M Y') }}</td>
                                <td class="px-4 py-3 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        <flux:button wire:click="openModal('{{ $agent->id }}')" size="sm" variant="ghost"
                                            icon="pencil" />
                                        <flux:button wire:click="confirmDelete('{{ $agent->id }}')" size="sm"
